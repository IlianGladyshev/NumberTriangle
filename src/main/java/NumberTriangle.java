...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        ...
        NumberTriangle rootElement = this;
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        ArrayList<NumberTriangle> topLayer = new ArrayList<NumberTriangle>();
        ArrayList<NumberTriangle> bottomLayer = new ArrayList<NumberTriangle>();
        ...
        while (line != null) {
            ...
            if (nodes.length == 1) {
                top = new NumberTriangle(Integer.parseInt(nodes[0]));
                topLayer.add(top);
            }
            else {
                for (int i = 0; i < nodes.length; i++) {
                    bottomLayer.add(new NumberTriangle(Integer.parseInt(nodes[i])));
                }
                for (int i = 0; i < topLayer.size(); i++) {
                    topLayer.get(i).left = bottomLayer.get(i);
                    topLayer.get(i).right = bottomLayer.get(i + 1);
                }
                topLayer = new ArrayList<NumberTriangle>();
                topLayer.addAll(bottomLayer);
                bottomLayer = new ArrayList<NumberTriangle>();
            }
